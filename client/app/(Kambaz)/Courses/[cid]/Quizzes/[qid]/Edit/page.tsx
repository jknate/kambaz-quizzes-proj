...
const [editingNewQuestion, setEditingNewQuestion] = useState<any>(null);
...
const [selectedType, setSelectedType] = useState("TRUE_FALSE");
    ...
    <Button
      ...
      onClick={() => {
        if (selectedType === "TRUE_FALSE") {
          const q = newTrueFalseQuestion();
          setEditingNewQuestion(q);
        }
      }}
    >
      + Add Question
  ...
  {editingNewQuestion && (
    <TrueFalseQuestionEditor
      question={editingNewQuestion}
      updateQuestion={(q: any) => setEditingNewQuestion(q)}
      
     onSave={(q: any) => {
        const qs = (quiz.questions || []).slice();
        const ix = qs.findIndex((x: any) => x._id === q._id);
        if (ix !== -1) qs[ix] = q;
        else qs.push(q);
        setQuiz({ ...quiz, questions: qs });
        setEditingNewQuestion(null);
      }}
      onCancel={() => {
        if (!editingNewQuestion) return;

        setQuiz({
          ...quiz,
          questions: (quiz.questions || []).filter((x: any) => x._id !== editingNewQuestion._id),
        });
        setEditingNewQuestion(null);
      }}
   />
